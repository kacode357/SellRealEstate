...
export default function Customerdondat() {
    ...
    const getEmailByCustomerId = (customerId) => {
        const account = accounts.find(item => item.id === customerId);
        return account ? account.email : 'Unknown';
    };
    const getPhoneByCustomerId = (customerId) => {
        const account = accounts.find(item => item.id === customerId);
        return account ? account.phoneNumber : 'Unknown';
    };
                ...
                {customerReservation.length > 0 ? (
                    customerReservation.map((reservation, index) => (
                        <div key={index} className=''>
                            ...
                            <p><b>Email: {getEmailByCustomerId(reservation.agencyId)}</b></p>
                            <p><b>SĐT: {getPhoneByCustomerId(reservation.agencyId)}</b></p>
