            ...
            if (selectedRealEstateId) {
                ...
                await Promise.all(bookingsToUpdate.map(async booking => {
                    ...
                    const notificationData = {
                        accountId: booking.customerId, // Assuming customerId is the user's account ID
                        description: `Bất động sản ${getRealEstateNameById(booking.realEstateId)} vào lúc ${timeSlot} ngày ${formatDate(date)} đã được điều phối nhân viên`
                    };
    
                    // Send notification to the user
                   CallApi.createNotification(notificationData);
                }));
