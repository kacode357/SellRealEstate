...
export default function AgencyDatcocmuaban() {
    ...
    const loc = (realEstateId) => {
        const realEstate = realEstates.find(item => item.id === realEstateId);
        return realEstate ? (realEstate.reservationId) : 'Unknown';
    }
    const loc2 = (realEstateId, resvervationId) => {

        //    loc(realEstateId) === resvervationId
        if (loc(realEstateId) === resvervationId) {
            if (getStatusByRealEstateId(realEstateId) === 6) {
                return 2;
            }
            return 1;
        } if (getStatusByRealEstateId(realEstateId) === 6) {
                                ...
                                {filteredReservations.map((reservation, index) => (
                                    getStatusByRealEstateId(reservation.realEstateId) !== 0 && (
                                        <tr key={index}>
                                            ...
                                            <td>{getStatus(loc2(reservation.realEstateId, reservation.id))}</td>
                                            <td >
