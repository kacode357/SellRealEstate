...
export default function AdminDetailBookingAgen() {
    ...
    useEffect(() => {
        // Check if a new date is selected to toggle filtering
        ...
        if (window.confirm("Bạn có chắc chắn muốn đánh dấu tất cả các đơn đặt chỗ này là đã hoàn thành không?")) {
            try {
                await Promise.all(reservationsToMarkComplete.map(async reservation => {
                    ...
                    const thongbao = {
                        accountId: reservation.customerId,
                        description: `Đơn đặt chỗ của bạn cho bất động sản ${getRealEstateNameById(reservation.realEstateId)} vào lúc ${reservation.bookingTime} ngày ${formatDate(reservation.bookingDate)} đã được hoàn thành.`
                    }
        ...
        <div className='admin-all-account'>
            ...
            <div className='box-allaccount'>
                ...
                <div className="account-list">
                    ...
                    <div>
                        ...
                        <div style={{ margin: '10px' }}>
                            <button className="btn btn-outline-success" onClick={toggleFilter}>{isFiltering ? 'Hiển thị tất cả' : 'Lọc'}</button>
                        </div>
                    </div>
                </div>
                <div className="account-list">
                ...
                {(isFiltering === true && filteredBookReservations.length > 0) ? (
                    <div style={{ margin: '10px' }}>
                        <button className="btn btn-outline-success" onClick={handleCompleteClick}>Đánh dấu tất cả đã hoàn thành</button>
                    </div>
                ) : (
                    <p></p>
                )}
